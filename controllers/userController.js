    ...
    async (req, res) => {
        ...
        try {
            ...
            if(rows.length > 0){
                ...
                const userCreated = await pool.query('INSERT INTO users(first_name, last_name, email, hash, salt, status) VALUES ($1, $2, $3, $4, $5, 0) RETURNING id;', 
                    [req.body.firstName, req.body.lastName, req.body.email, hash, salt]);
                
                if(userCreated.rows.length > 0){
                    console.log(userCreated);
                    await pool.query('INSERT INTO cart(user_id) VALUES ($1);', [userCreated.rows[0].id]);
                }
